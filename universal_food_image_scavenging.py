...
FOOD_IMAGE_LIBRARY = {
    ...
    "burrito": {"general": "https://images.unsplash.com/photo-1565299507177-b0ac66763828?w=1080&h=720&fit=crop&q=80"},
...
def get_image_for_recipe(recipe_name, recipe_category="", ingredients=[], recipe_id=0):
    ...
    for subcategory, url in category_images.items():
        if subcategory != "general" and subcategory in recipe_lower:
            return {"url": url, "category": main_category, "subcategory": subcategory, "match_type": "specific"}
    ...
    return {"url": selected_url, "category": main_category, "subcategory": "general", "match_type": "category_general"}
...
def process_recipes_batch(db, recipes, batch_size=50):
    ...
    for i in range(0, total_recipes, batch_size):
        ...
        print(
            f"      Processed: {len(batch)}, Success: {len([r for r in batch if test_image_url(get_image_for_recipe(r.get('Name', ''), '', [], r.get('RecipeId', 0))['url'])])}"
        )
    ...
    return {"processed": processed, "successful": successful, "failed": failed, "category_stats": dict(category_stats)}
    ...
    try:
        ...
        for recipe in recipes_without_images[:20]:  # Sample analysis
            category = detect_food_category(
                recipe.get("Name", ""), recipe.get("RecipeCategory", ""), recipe.get("RecipeIngredientParts", [])
            )
