...
def extract_keywords_from_recipe(recipe: Dict[str, Any]) -> List[str]:
    ...
    stop_words = {
        'recipe', 'easy', 'quick', 'simple', 'best', 'homemade', 'perfect', 
        'delicious', 'minutes', 'prep', 'cook', 'time', 'serving', 'serves',
        'ingredients', 'instructions', 'step', 'add', 'mix', 'cook', 'bake'
    }
    ...
    for keyword in keywords:
        ...
        if (clean_keyword not in stop_words and 
            len(clean_keyword) > 2 and 
            clean_keyword not in seen and
            clean_keyword.isalpha()):
