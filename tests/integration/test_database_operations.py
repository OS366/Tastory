...
from app import get_top_review, log_search_query, calculate_trending_searches
...
class TestRecipeDatabase:
    ...
    def test_recipe_search_by_category(self, populated_db):
        ...
        results = list(recipes_collection.find(
            {"RecipeCategory": {"$regex": "indian", "$options": "i"}}
        ))
...
class TestReviewDatabase:
    ...
    def test_reviews_by_rating(self, populated_db):
        ...
        high_rated = list(reviews_collection.find(
            {"Rating": {"$gte": 4}}
        ))
    ...
    def test_reviews_sorting(self, populated_db):
        ...
        sorted_reviews = list(reviews_collection.find(
            {"RecipeId": 1}
...
class TestComplexQueries:
    ...
    def test_multi_field_search(self, populated_db):
        ...
        for recipe in results:
            contains_chicken = (
                "chicken" in recipe.get("Name", "").lower() or
                any("chicken" in str(ing).lower() for ing in recipe.get("RecipeIngredientParts", [])) or
                "chicken" in recipe.get("RecipeCategory", "").lower()
            )
    ...
    def test_aggregation_pipeline(self, populated_db):
        ...
        pipeline = [
            {"$group": {
                "_id": "$RecipeCategory",
                "count": {"$sum": 1},
                "avg_rating": {"$avg": "$AggregatedRating"}
            }},
            {"$sort": {"count": -1}}
        ]
