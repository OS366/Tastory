    ...
    def load_progress(self):
        ...
        try:
            if os.path.exists(self.filename):
                ...
                print(f"Loaded progress: {len(self.processed_recipes)} recipes and {len(self.processed_reviews)} reviews processed")
            else:
        ...
        try:
            with open(self.filename, 'wb') as f:
                pickle.dump({
                    ...
                    'reviews': self.processed_reviews
                }, f)
        except Exception as e:
...
def prepare_recipe_document(recipe):
    ...
    for key, value in doc.items():
        if pd.isna(value):
            ...
            doc[key] = [
                item.item() if isinstance(item, (np.int64, np.float64)) else item
                for item in value
            ]
