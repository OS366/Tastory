...
def estimate_cost(collection):
    ...
    query = {"$or": [{RECIPE_EMBEDDING_FIELD: {"$exists": False}}, {RECIPE_EMBEDDING_FIELD: []}]}
...
def process_recipes_embeddings(collection, model):
            ...
            try:
                ...
                for idx, response in enumerate(responses):
                    if hasattr(response, "values") and response.values:
                        ...
                        operations.append(
                            pymongo.UpdateOne(
                                {"_id": doc["_id"]}, {"$set": {RECIPE_EMBEDDING_FIELD: response.values}}, upsert=False
                            )
                        )
                    else:
                        error_count += 1
                ...
                if operations:
                    ...
                    print(
                        f"\rProcessed {processed_count}/{total_docs_to_process} recipes. Cost so far: ${current_cost:.2f}",
                        end="",
                    )
    ...
    sample_embedded_docs = collection.find({RECIPE_EMBEDDING_FIELD: {"$exists": True, "$ne": []}}).limit(3)
...
def verify_embeddings(collection):
    ...
    docs_with_embeddings = collection.count_documents({RECIPE_EMBEDDING_FIELD: {"$exists": True, "$ne": []}})
    ...
    sample_docs = collection.find({RECIPE_EMBEDDING_FIELD: {"$exists": True, "$ne": []}}).limit(3)
