...
def estimate_cost(collection):
    ...
    query = {"$or": [{REVIEW_EMBEDDING_FIELD: {"$exists": False}}, {REVIEW_EMBEDDING_FIELD: []}]}
...
def process_reviews_embeddings(collection, model):
            ...
            try:
                ...
                for idx, response in enumerate(responses):
                    if hasattr(response, "values") and response.values:
                        ...
                        operations.append(
                            pymongo.UpdateOne(
                                {"_id": doc["_id"]}, {"$set": {REVIEW_EMBEDDING_FIELD: response.values}}, upsert=False
                            )
                        )
                    else:
                        error_count += 1
                ...
                if operations:
                    ...
                    print(
                        f"\rProcessed {processed_count}/{total_docs_to_process} reviews. Cost so far: ${current_cost:.2f}",
                        end="",
                    )
    ...
    sample_embedded_docs = collection.find({REVIEW_EMBEDDING_FIELD: {"$exists": True, "$ne": []}}).limit(3)
