...
def get_overall_stats(db):
    ...
    recipes_with_images = recipes_collection.count_documents({
        "MainImage": {"$exists": True, "$ne": None, "$ne": "", "$not": {"$regex": "^\\s*$"}}
    })
    ...
    scavenged_recipes = recipes_collection.count_documents({
        "UnsplashData.food_scavenging": True
    })
    
    # Universal system recipes
    universal_system_recipes = recipes_collection.count_documents({
        "UnsplashData.universal_system": True
    })
...
def get_category_breakdown(db):
    ...
    scavenged_recipes = recipes_collection.find({
        "UnsplashData.food_scavenging": True,
        "UnsplashData.category": {"$exists": True}
    }, {"UnsplashData.category": 1})
...
def check_broken_images_sample(db, sample_size=50):
    ...
    recipes_with_images = list(recipes_collection.find({
        "MainImage": {"$exists": True, "$ne": None, "$ne": ""}
    }, {"RecipeId": 1, "Name": 1, "MainImage": 1}).limit(sample_size))
...
def get_recent_activity(db, days=7):
    ...
    recent_scavenged = recipes_collection.count_documents({
        "UnsplashData.added_at": {"$gte": days_ago}
    })
    
    return {
        'days': days,
        'recent_additions': recent_scavenged
    }
